...
extern "C" {
    ...
    // NUMA weight mirror: the CPU backend hands the scheduler its per-node remap
    // function at init (ggml_backend_cpu_init), so ggml-base never has to reach
    // upward into the registry to find it. fn(ptr, node) returns the address of
    // ptr's bytes in the replica local to `node`, or ptr itself if not mirrored.
